...
import { Card, CardContent, CardHeader } from "@/components/ui/card"; // Correct import
...
export function FAQ() {
    ...
    const accordionItemVariants = { hidden: { opacity: 0, x: -10 }, visible: { opacity: 1, x: 0 } };
    ...
    return (
        <Section id="faq" className="py-20 md:py-28 lg:py-32 bg-gradient-to-b from-muted/10 to-background dark:from-background/10 dark:to-background">
            ...
            >
                {/* Use Card with paddingless prop */}
                <Card paddingless className="overflow-hidden border border-border/15 dark:border-border/20">
                                ...
                                {categories.map(cat => (
                                    <Button
                                        ...
                                        className={cn(
                                            ...
                                            activeCategory === cat.value && "bg-primary hover:bg-primary/90 text-primary-foreground", // Already default styles
                                            activeCategory !== cat.value && "border-border/50 text-muted-foreground hover:bg-accent hover:text-accent-foreground" // Already default styles
                                        )}
                    ...
                    {/* Apply padding directly to CardContent as Card is paddingless */}
                                ...
                                <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ staggerChildren: 0.07 }}>
                                    {filteredFaqs.map((item) => (
                                        <motion.div key={item.id} layout variants={accordionItemVariants} initial="hidden" animate="visible">
                                            <AccordionItem
                                                value={`item-${item.id}`}
                                                // Contextual state/hover styles OK
                                                ...
                                                <AccordionContent className="text-muted-foreground text-sm px-4 pb-4 pt-1 leading-relaxed space-y-2">
                                                    ...
                                                    <div className="flex flex-wrap gap-1 pt-2">
                                                        {item.tags.slice(0, 5).map(tag => (
                                                            // Contextual hover interaction OK
