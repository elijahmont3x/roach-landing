...
import { BookOpen, Coins, ExternalLink, FileText, FlaskConical, GitBranch, HelpCircle, Lock, ShieldCheck } from "lucide-react"; // Added Lock icon
...
export function Footer() {
    ...
    return (
        <footer className="bg-muted/40 dark:bg-background/30 border-t border-border/50 mt-24"> {/* OK: Layout color/border */}
            <TooltipProvider delayDuration={150}>
                <div className="container mx-auto pt-16 pb-8 px-4 md:px-6"> {/* OK: Layout */}
                    <div className="grid grid-cols-1 gap-10 md:grid-cols-6 lg:grid-cols-12 items-start"> {/* OK: Layout */}

                        {/* Brand & Socials (Span 4 cols) */}
                        <div className="md:col-span-3 lg:col-span-4 space-y-4"> {/* OK: Layout */}
                            <Link href="/" className="flex items-center gap-2.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-sm w-fit group"> {/* OK: Styling on link, not Button */}
                                <CockroachMascot size="sm" className="transition-transform group-hover:scale-110" /> {/* OK: Non-conflicting transform */}
                                <span className="text-xl font-bold">$ROACH</span>
                            </Link>
                            <p className="text-sm text-muted-foreground max-w-xs leading-relaxed"> {/* OK: Specific layout text style */}
                            ...
                            <div className="flex items-center gap-1.5 pt-2"> {/* OK: Layout spacing */}
                        ...
                        <div className="md:col-span-3 lg:col-span-2 space-y-3"> {/* OK: Layout */}
                            <h4 className="text-sm font-semibold uppercase tracking-wider text-foreground/90 mb-3">Navigate</h4> {/* OK: Heading style */}
                            <ul className="space-y-2 text-sm"> {/* OK: Layout */}
                                {NAV_LINKS.map(link => (
                                    <li key={link.href} className="flex items-center"> {/* OK: Layout */}
                                        <link.icon className="h-3.5 w-3.5 mr-2 text-muted-foreground/70" /> {/* OK: Icon style */}
                                        {/* FooterLink handles link styling correctly */}
                        ...
                        <div className="md:col-span-3 lg:col-span-3 space-y-3"> {/* OK: Layout */}
                            <h4 className="text-sm font-semibold uppercase tracking-wider text-foreground/90 mb-3">Resources</h4> {/* OK: Heading style */}
                            <ul className="space-y-2 text-sm"> {/* OK: Layout */}
                        ...
                        <div className="md:col-span-3 lg:col-span-3 space-y-3"> {/* OK: Layout */}
                            <h4 className="text-sm font-semibold uppercase tracking-wider text-foreground/90 mb-3">Get $ROACH</h4> {/* OK: Heading style */}
                            <ul className="space-y-2 text-sm flex flex-col items-start"> {/* OK: Layout */}
                                ...
                                <li className="mt-2">
                                    {/* Refactored Button as link */}
                                    <Link href="#how-to-buy" className={cn(buttonVariants({ variant: 'link', size: 'sm' }), "text-xs h-auto p-0 text-primary hover:underline")}>
                    ...
                    <Separator className="my-10 bg-border/30 dark:bg-border/20" /> {/* OK: Layout color override */}

                    {/* Bottom Bar: Copyright & Minimal Links */}
                    <div className="flex flex-col items-center justify-between gap-4 text-xs text-muted-foreground md:flex-row"> {/* OK: Layout */}
                    ...
                    {/* Enhanced Disclaimer */}
                    <div className="mt-8 pt-8 border-t border-border/30 dark:border-border/20"> {/* OK: Layout */}
                        <p className="text-[0.75rem] leading-relaxed text-muted-foreground/80 text-center max-w-4xl mx-auto"> {/* OK: Layout/Text Style */}
...
// OK: This defines a specific link style used only here.
function FooterLink({ href, children, target }: { href: string; children: React.ReactNode; target?: string }) {
    const isExternal = target === "_blank" || href.startsWith('http'); // Check if external
    const isInternalSection = href.startsWith('#'); // Check if section link
    ...
    const componentProps = isInternalSection
        ...
        : { href: href, target: target, rel: isExternal ? "noopener noreferrer" : undefined };
    ...
    return (
        <Link
            ...
            className={cn(
                ...
                "inline-flex items-center gap-1.5 group transition-colors duration-200" // OK: Layout/Group Interaction
            )}
            aria-label={`View ${children}`}
        >
            <Icon className="h-3.5 w-3.5 text-muted-foreground/70 group-hover:text-foreground transition-colors" /> {/* OK: Icon color */}
            <span className="group-hover:underline underline-offset-4">{children}</span> {/* OK: Group interaction */}
            <ExternalLink className="h-3 w-3 opacity-0 group-hover:opacity-60 transition-opacity duration-200" /> {/* OK: Group interaction */}
        </Link>
    );
}
...
// OK: Uses Button component correctly with variant="ghost", size="icon".
function SocialLink({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) {
    return (
        <Tooltip>
            <TooltipTrigger asChild>
                {/* Correct usage: Button component inside Link */}
                <Link href={href} target="_blank" rel="noopener noreferrer" aria-label={label}>
                     <Button
                        variant="ghost"
                        size="icon"
                        // Removed hover/focus overrides, rely on base Button style. Added focus-offset for footer bg
                        className={cn(
...
// OK: Uses Button correctly with variant="outline", size="sm". Adds custom hover styles and dark mode styles that don't override core structure.
function SwapLinkButton({ href, children }: { href: string; children: React.ReactNode; }) {
    return (
        // Corrected usage: Button inside Link
         <Link href={href} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto">
            <Button variant="outline" size="sm" className={cn(
                "w-full sm:w-auto justify-start sm:justify-center", // Removed shadow-sm, hover:border-primary/50, let Button handle base style
                "dark:bg-card dark:hover:bg-muted/50" // OK: Minor dark mode tweaks
            )}>
                <span className="flex items-center gap-2"> <Coins className="h-3.5 w-3.5 text-primary" /> </span> {/* OK: Example icon */}
                {children}
